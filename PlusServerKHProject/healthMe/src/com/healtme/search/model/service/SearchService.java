...
public class SearchService {
	...
	public SearchResult searchBar(ArrayList<String> searchList, int currentPage) {
		...
		if(!tmpTrainer.isEmpty()) {
			...
			//1번에서 찾아온 트레이너 아이디와 일치하는 결과를 가져옴
			//결과를 저장할 ArrayList
			...
			if(startNavi==1) {//시작 페이지가 1페이지라면 ◀표시가 나오지 않도록 함
				 sb.append("<a href = '/searchInput.do?currentPage=" + (startNavi-1) + "'> ◀ </a>&nbsp&nbsp&nbsp");
			}
			for(int i = startNavi ; i <= endNavi ; i++) {//페이지 번호 표시
				if(i == currentPage) {//현재 페이지가 사용자의 위치페이지와 같다면 배경색 표시
					sb.append("<a href = '/searchInput.do?currentPage=" + i + "'><i style='background:#FF9B55'>" + i + "</i></a>&nbsp&nbsp&nbsp");
				}else {//사용자의 현재 위치 페이지가 아닌경우 일반 표시
					sb.append("<a href = '/searchInput.do?currentPage=" + i + "'>" + i + "</a>&nbsp&nbsp&nbsp");
				}
			}
			if(endNavi == naviTotalNum) {
				sb.append("<a href='/searchInput.do?currentPage=" + (endNavi+1) + "'> ▶ </a>&nbsp&nbsp&nbsp");
			}
			sb.append("</center>");
			...
			//3. 객체에 결과 저장
			searchResult = new SearchResult(trainerList, sb.toString());
		}
